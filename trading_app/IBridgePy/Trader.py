...
from IBridgePy.IbridgepyTools import special_match, create_order, check_same_security, from_symbol_to_security, \
    print_decisions
from IBridgePy.constants import OrderStatus, FollowUpRequest, OrderType, OrderTif
...
class Trader(object):
    ...
    tick_type_mapper = TICK_TYPE_MAPPER
    accountCodeSet = None
    ...
    def get_contract_details(self, secType, symbol, field, currency='USD', exchange='', primaryExchange='', expiry='',
        ...
        return self._brokerService.get_contract_details(security, field)

    def get_scanner_results(self, **kwargs):
        return self._brokerService.get_scanner_results(kwargs)
    ...
    def get_option_greeks(self, security, tickType, fields):
        ...
        return self._brokerService.get_option_greeks(security, self.tick_type_mapper[tickType], fields)
    ...
    def processMessages(self, timeNow):
        ...
        self.runRepeatFunctionFlag = self._brokerService.processMessages(timeNow)
    ...
    def repeat_Function(self, dummyTime):
        self._log.notset(__name__ + '::repeat_Function: dummyTime=%s self.runRepeatFunctionFlag=%s' % (dummyTime, self.runRepeatFunctionFlag))
    ...
    def request_historical_data(self, security, barSize, goBack, endTime='', whatToShow='', useRTH=1, formatDate=1,
        ...
        if self._dataProviderService:
            ...
            return self._dataProviderService.get_historical_data(security, barSize, goBack, endTime, whatToShow, useRTH,
                                                                 formatDate, waitForFeedbackInSeconds, timezoneOfReturn)
        else:
            self._brokerService.add_exchange_primaryExchange_to_security(security)
            return self._brokerService.get_historical_data(security, barSize, goBack, endTime, whatToShow, useRTH, formatDate, waitForFeedbackInSeconds, timezoneOfReturn)
    ...
    def show_real_time_price(self, security, param):
        ...
        self._dataProviderService.add_exchange_primaryExchange_to_security(security)
        return self._dataProviderService.get_real_time_price(security, self.tick_type_mapper[param])
    ...
    def show_real_time_size(self, security, param):
        self._log.info(__name__ + '::show_real_time_price: security=%s param=%s' % (str(security), str(param)))
        self._dataProviderService.add_exchange_primaryExchange_to_security(security)
        return self._dataProviderService.get_real_time_size(security, self.tick_type_mapper[param])
    ...
    def order_percent(self, security, percent, style=MarketOrder(), orderRef='', followUp=True,
        ...
        if percent > 1.0 or percent < -1.0:
            self._log.error(__name__ + '::order_percent: EXIT, percent=%s [-1.0, 1.0]' % (str(percent),))
            exit()
        portfolio_value = self._brokerService.get_account_info(adj_accountCode, 'NetLiquidation')
        targetShare = int(portfolio_value / self.show_real_time_price(security, 'ask_price'))
    ...
    def order_target_percent(self, security, percent, style=MarketOrder(), orderRef='', followUp=True,
                             accountCode='default'):
        self._log.debug(__name__ + '::order_target_percent')
        adj_accountCode = self.adjust_accountCode(accountCode)
        if percent > 1.0 or percent < -1.0:
            self._log.error(__name__ + '::order_target_percent: EXIT, percent=%s [-1.0, 1.0]' % (str(percent),))
            exit()
        a = self._brokerService.get_account_info(adj_accountCode, 'NetLiquidation')
        b = self.show_real_time_price(security, 'ask_price')
        if math.isnan(b):
            self._log.error(__name__ + '::order_target_percent: EXIT, No real time price is available right now. Market closed? real_time_price is NaN security=%s' % (security.full_print(),))
            exit()
        if b <= 0.0:
            self._log.error(__name__ + '::order_target_percent: EXIT, No real time price is available right now. Market closed? real_time_price=%s security=%s' % (b, security.full_print()))
            exit()
    ...
    def _helper(self, security, amount, style=MarketOrder(), accountCode='default'):
        self._brokerService.add_exchange_primaryExchange_to_security(security)
    ...
    def place_order_with_stoploss(self, security, amount, stopLossPrice, style=MarketOrder(), tif='DAY',
                                  accountCode='default'):
        # https://developer.tdameritrade.com/content/place-order-samples Not implemented for TD yet
        adj_accountCode, parentOrder, int_slOrderId, ocaGroup, int_parentOrderId = self._helper(security, amount, style, accountCode)
    ...
    def place_order_with_takeprofit(self, security, amount, takeProfitPrice, style=MarketOrder(), tif='DAY',
                                    accountCode='default'):
        # https://developer.tdameritrade.com/content/place-order-samples Not implemented for TD yet
        adj_accountCode, parentOrder, int_tpOrderId, ocaGroup, int_parentOrderId = self._helper(security, amount, style, accountCode)
    ...
    def place_order_with_stoploss_takeprofit(self, security, amount, stopLossPrice, takeProfitPrice,
                                             style=MarketOrder(), tif='DAY', accountCode='default', ocaGroup=None):
        """
        # https://developer.tdameritrade.com/content/place-order-samples Not implemented for TD yet
